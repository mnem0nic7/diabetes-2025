...
class Config:
    ...
    n_splits: int = 5
    seed: int = 42
    out: str = "submission_v16_shift_piecewise.csv"
    ...
    # Meta / gating
    meta_bins: int = 8
    meta_C_grid: tuple[float, ...] = (0.08, 0.15, 0.25, 0.5, 1.0)
...
def _train_domain_classifier(cfg: Config, train_X: pd.DataFrame, test_X: pd.DataFrame):
    """Train a train-vs-test classifier to estimate p(test | x) (cross-fit)."""
    ...
    p_test = np.clip(oof, 1e-4, 1 - 1e-4)
    return p_test[: len(train_X)], p_test[len(train_X) :]
...
def _add_orig_mean_count_features(train: pd.DataFrame, test: pd.DataFrame, orig_csv: str):
    ...
    Leakage-safe: orig stats come only from external data.
    """
...
def _meta_features(
    ...
    # Stable logit version (often more linear for LR)
    df["p_test_logit"] = np.log(p_test / (1.0 - p_test))

    # Piecewise gating: learn different expert weights per p_test bin
    # (acts like a soft mixture-of-experts conditioned on shift level)
    bin_idx = pd.qcut(df["p_test"], q=n_bins, labels=False, duplicates="drop")
    bin_idx = bin_idx.astype(int)
    dummies = pd.get_dummies(bin_idx, prefix="bin", dtype=float)

    # Interaction terms: expert prediction * bin_onehot
    for base_col in ["p_lgb_raw", "p_lgb_orig", "p_cat"]:
        for bcol in dummies.columns:
            df[f"{base_col}__{bcol}"] = df[base_col].values * dummies[bcol].values

    df = pd.concat([df, dummies], axis=1)

    if pa is not None:
        df["physical_activity_minutes_per_week"] = pa
...
def _tune_meta_C(
    ...
    for C in C_grid:
        oof = np.zeros(len(meta_train), dtype=np.float64)
        for tr_idx, va_idx in cv.split(meta_train, y, groups=groups):
            scaler = StandardScaler()
            X_tr = scaler.fit_transform(meta_train.iloc[tr_idx].values)
            X_va = scaler.transform(meta_train.iloc[va_idx].values)
        ...
        print(f"Meta tune: C={C:.3f} | OOF AUC={auc:.5f}")
        if auc > best_auc:
            best_auc = auc
            best_C = C
...
def main():
    p = argparse.ArgumentParser(description="V16: shift-aware stacking with piecewise meta gating")
    ...
    if pa_col in X_train_base.columns:
        med = X_train_base[pa_col].median()
        pa_train = X_train_base[pa_col].astype(float).fillna(med).to_numpy()
        pa_test = X_test_base[pa_col].astype(float).fillna(med).to_numpy()

    meta_train = _meta_features(
        oof_raw=oof_lgb_raw,
        oof_orig=oof_lgb_orig,
        oof_cat=oof_cat,
        p_test=p_test_train,
        pa=pa_train,
        n_bins=cfg.meta_bins,
    )
    meta_test = _meta_features(
        oof_raw=test_lgb_raw,
        oof_orig=test_lgb_orig,
        oof_cat=test_cat,
        p_test=p_test_test,
        pa=pa_test,
        n_bins=cfg.meta_bins,
    ...
    best_C, best_auc = _tune_meta_C(
        meta_train=meta_train,
        y=y,
        cv=cv,
        groups=groups,
        C_grid=cfg.meta_C_grid,
        seed=cfg.seed,
    )
    print("=" * 60)
    print(f"Meta best: C={best_C:.3f} | OOF AUC={best_auc:.5f}")
